...
if ($link === false){
	...
	if (($res_bet = mysqli_query($link, $sql_bet)) and ($res_lot = mysqli_query($link, $sql_lot)) and ($res_cat = mysqli_query($link, $sql_cat))) 
    {
        ...
        if(empty($bets)){
            $page_content = include_template('no_bet.php', ['lots' => $lots, 'bets' => $bets, 'category' => $category]);
        }   
        else{
